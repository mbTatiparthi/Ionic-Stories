...
/** onesignal **/
$config["onesignal_app_id"]				= "f8d14c99-30ed-43ed-ad07-cac965f4356d" ; //Your OneSignal AppId, available in OneSignal https://documentation.onesignal.com/docs/generate-a-google-server-api-key
$config["onesignal_api_key"]			= "MTY5NWYwNzctOTQ4MC00NzlmLTg3MDUtMDA2NDMzODEwMzkx" ; //Your OneSignal ApiKey, required for push notification sender
...
switch($_GET["page"]){
	...
	case "home":
		...
		$content .= '<a href="?">';
		$content .= '<i class="fa fa-book"></i> <span>Stories</span>';
		$content .= '<span class="pull-right-container">';
		$content .= '<i class="fa fa-angle-left pull-right"></i>';
		$content .= '</span>';
		$content .= '</a>';
		...
		$content .= '</div>';
		$content .= '<strong>Copyright &copy; '.date("Y").' <a href="http://godigitally.co.in">Go Digitally.co.in</a>.</strong> All rights reserved.';
		$content .= '</footer>';
		$content .= '</div>';
		break;
	// TODO: PAGE - LOGIN
	case "login":
		...
		$content .= '<img src="'.$config["logo"].'?1605456129" />';
		...
		switch($_GET["action"]){
			case "list":
				...
				if($result = $mysql->query($sql_query)){
					while ($data = $result->fetch_array()){
						...
						/** story_category **/
						$categories_text = htmlentities(stripslashes($data["story_category"]));
						...
						$content .= '<a class="btn btn-danger btn-flat btn-sm" href="#" onClick="doModal(\'Delete Story\',\'<div class=\\\'row\\\'><div class=\\\'col-md-3 text-center text-primary\\\'><i class=\\\'fa fa-5x fa-book\\\'></i></div><div class=\\\'col-md-9\\\'>You are about to permanently delete these items from your site. <br/>This action cannot be undo, `Cancel` to stop, `OK` to delete.</div></div>\',\'Ok\',\'danger\',\'window.location=\\\'?page=stories&amp;action=delete&amp;id='.$data["id"].'\\\'\');"><i class="fa fa-trash"></i></a>';
						$content .= '</td>';
						$content .= '</tr>';
					}
				}
			...
			case "edit":
				...
				if(isset($_GET["id"])){
					...
					if(isset($rowdata["id"])){
						...
						$postdata["story-category"] = "" ;
						$postdata["story-date"] = "" ;
						$postdata["story-details"] = "" ;
						/** response postdata **/
						if(isset($_POST["submit"])){
							...
							if(isset($_POST["postdata"]["story-category"])){
								$postdata["story-category"] = addslashes($_POST["postdata"]["story-category"]);
							}
							if(isset($_POST["postdata"]["story-date"])){
								$postdata["story-date"] = addslashes($_POST["postdata"]["story-date"]);
							}
							if(isset($_POST["postdata"]["story-details"])){
								$postdata["story-details"] = addslashes($_POST["postdata"]["story-details"]);
							}
							$sql_query = "UPDATE `stories` SET `story_title` = '{$postdata["story-title"]}' ,`story_image` = '{$postdata["story-image"]}' ,`story_category` = '{$postdata["story-category"]}' ,`story_date` = '{$postdata["story-date"]}' ,`story_details` = '{$postdata["story-details"]}'  WHERE `id`=$entry_id" ;
							$stmt = $mysql->prepare($sql_query);
							$stmt->execute();
							$stmt->close();
						...
						$postdata["story-category"] = '';
						if(isset($rowdata["story_category"])){
							$postdata["story-category"] = stripslashes($rowdata["story_category"]);
						}
						$postdata["story-date"] = '';
						...
						$postdata["story-details"] = '';
						if(isset($rowdata["story_details"])){
							$postdata["story-details"] = stripslashes($rowdata["story_details"]);
						}
						/** breadcrumb **/
						$content .= '<section class="content-header">';
						$content .= '<h1>Stories <small></small></h1>';
						...
						$content .= '</div>';
						$content .= '</div>';
						/** field story_category:select-table **/
						$options["story_category"] = array();
						...
						if($option_result){
							while ($option_data = $option_result->fetch_array()){
								$options["story_category"][] = array("val"=> $option_data["id"],"label"=>$option_data["category_name"]);
							}
						}else{
							$options["story_category"][] = array("val"=> "","label"=>"Not existing table");
						}
						$content .= '<div class="col-md-6">';
						$content .= '<div class="form-group">';
						$content .= '<label>Story Category</label>';
						$content .= '<select class="form-control" name="postdata[story-category]" id="postdata-story-category">';
						foreach($options["story_category"] as $option) {
							$selected ="";
							if($option["val"] == $postdata['story-category'] ){
								$selected ="selected";
							}
							$content .= '<option value="'.htmlentities($option["val"]).'" '.$selected.'>'.htmlentities($option["label"]).'</option>';
						}
						$content .= '</select>';
						$content .= '<p class="help-block">Story Category</p>';
						$content .= '</div>';
						$content .= '</div>';
						...
						$content .= '</div>';
						$content .= '</div>';
						/** field story_details:longtext **/
						$content .= '<div class="col-md-12">';
						$content .= '<div class="form-group">';
						$content .= '<label>Story Details</label>';
						$content .= '<textarea name="postdata[story-details]" id="postdata-story-details" class="form-control" data-type="html5" >'.htmlentities(stripslashes($postdata['story-details'])).'</textarea>';
						$content .= '<p class="help-block"></p>';
						$content .= '</div>';
						$content .= '</div>';
						$content .= '</div>';
			...
			case "add":
				...
				if(isset($_POST["submit"])){
					...
					$sql_query = "INSERT INTO `stories` (`story_title`,`story_image`,`story_category`,`story_date`,`story_details`) VALUES ('{$postdata['story-title']}','{$postdata['story-image']}','{$postdata['story-category']}','{$postdata['story-date']}','{$postdata['story-details']}')" ;
					$mysql->query($sql_query);
					$last_id = $mysql->insert_id;
...
$html_tags .= '<meta name="generator" content="IMA-BuildeRz vrev20.11.15" />';
